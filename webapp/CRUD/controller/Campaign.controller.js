sap.ui.define([
   ...
   "sap/m/MessageToast"
	...
	"use strict";
	return Controller.extend("gbi.controller.Campaign", {
	    onInit: function() {
	...
	handleEditPress : function () {
		...
		this._oCustomer = jQuery.extend({}, this.getView().byId("idCampaignForm").getBindingContext('gbi').getObject());
		...
		this._context =  this.getView().byId("idCampaignForm").getBindingContext('gbi');
		//Save the binding path so we can perform puts and posts
		this._sPath = this.getView().byId("idCampaignForm").getBindingContext('gbi').sPath.slice(1);
	...
	handleTableRowPress : function(oEvent){
        ...
        this.getView().byId("idCampaignForm").setBindingContext(context,'gbi');
    }, 
